...
<div class="filters-container">
    <h4>عدد الصفوف المرشحة: <span id="filteredRowCount"></span></h4>
    <h4>إجمالي عدد الصفوف: <span id="totalRowCount"></span></h4>
    ...
    <input type="text" id="Searchschool" placeholder="ابحث عن صف ...">
    <button id="Searchschoolbutton">بحث</button>

    <input type="text" id="Searchclass" placeholder="ابحث عن شعبة  ...">
    ...
    <table class="table table-bordered table-hover" name="myTable" id="myTable">
        <thead class="table-dark">
            ...
            <th>صف الطالب</th>
                ...
                <th>رقم التواصل الاول</th>
                <th>رقم التواصل الثاني</th>
        ...
        <td>{{ $item->parent_phone1 }}</td>
        <td>{{ $item->parent_phone2 }}</td>
   
        <td><a class="btn btn-outline-info" href="{{ route('studentshow.show_studen', $item->id) }}">Show</a></td>

<td>
            <form action="{{ route('student.destroy', $item->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من الحذف؟');">
                @csrf
                @method('DELETE')
